  ...
  return (
    <div className="min-h-screen bg-slate-50">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h1 className="text-xl font-bold text-slate-900">Planos e Assinaturas</h1>
              </div>
            </div>
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 space-y-6">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="flex items-center gap-2 font-bold text-slate-800">
              <Layers className="h-4 w-4 text-indigo-600" /> {editingId ? 'Editar plano' : 'Novo plano'}
            </h2>
            {editingId && (
          ...
          <form onSubmit={handleSubmit} className="flex flex-wrap items-end gap-3">
            ...
            <label className="flex items-center gap-2 cursor-pointer pb-2">
              ...
              />
              <span className="text-sm text-slate-600 flex items-center gap-1"><Star className="h-3.5 w-3.5 text-amber-500" /> Mais Popular</span>
            </label>

            <div className="w-full grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div>
                <label className="block text-xs text-slate-500 mb-1">Limite de pedidos/mês</label>
                <input
                  type="number" min={0}
              ...
              <div>
                <label className="block text-xs text-slate-500 mb-1">Limite de produtos</label>
                <input
                  type="number" min={0}
              ...
              <div>
                <label className="block text-xs text-slate-500 mb-1">Entregadores</label>
                <input
                  type="number" min={0}
            ...
            <div className="w-full">
              <label className="block text-xs text-slate-500 mb-1">Recursos inclusos (um por linha)</label>
              <textarea
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <h2 className="font-bold text-slate-800 mb-4">Planos cadastrados</h2>
          {loading ? (
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {planos.map((plano) => (
                <div
                  key={plano.id}
                  className={`relative border rounded-2xl p-4 ${plano.destaque ? 'border-indigo-300 ring-2 ring-indigo-100' : 'border-slate-200'}`}
                >
                  {plano.destaque && (
                    <span className="absolute -top-2.5 left-4 flex items-center gap-1 bg-indigo-600 text-white text-[10px] font-bold px-2.5 py-1 rounded-full">
                      <Star className="h-3 w-3" /> MAIS POPULAR
                    </span>
                  )}
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <p className="font-bold text-slate-800">{plano.nome}</p>
                      <p className="text-2xl font-bold text-indigo-600 mt-1">R$ {plano.valorMensal.toFixed(2)}<span className="text-xs font-normal text-slate-400">/mês</span></p>
                    ...
                    <button
                      ...
                      className={`shrink-0 px-2.5 py-1 rounded-full text-xs font-medium ${plano.ativo ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-200 text-slate-600'}`}
                    >
                      {plano.ativo ? 'Ativo' : 'Inativo'}
                    </button>
                  </div>
                  {plano.descricao && <p className="text-xs text-slate-400 mt-2">{plano.descricao}</p>}
                  ...
                  {(plano.limitePedidosMes || plano.limiteProdutos || plano.limiteUsuarios || plano.limiteEntregadores) && (
                    <div className="flex flex-wrap gap-1.5 mt-3">
                      {plano.limitePedidosMes != null && (
                        <span className="text-[11px] bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{formatLimite(plano.limitePedidosMes, 'pedidos/mês')}</span>
                      )}
                      {plano.limiteProdutos != null && (
                        <span className="text-[11px] bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{formatLimite(plano.limiteProdutos, 'produtos')}</span>
                      )}
                      {plano.limiteUsuarios != null && (
                        <span className="text-[11px] bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{formatLimite(plano.limiteUsuarios, 'usuários')}</span>
                      )}
                      {plano.limiteEntregadores != null && (
                        <span className="text-[11px] bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{formatLimite(plano.limiteEntregadores, 'entregadores')}</span>
                      )}
                    </div>
                  )}
                    ...
                    <ul className="mt-3 space-y-1 max-h-40 overflow-y-auto pr-1">
                      {plano.recursos.map((recurso, idx) => (
                        <li key={idx} className="flex items-start gap-1.5 text-xs text-slate-600">
                          <ListChecks className="h-3 w-3 mt-0.5 shrink-0 text-indigo-500" />
                          <span>{recurso}</span>
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className="flex items-center gap-2 mt-3 pt-3 border-t border-slate-100">
                    <button onClick={() => handleEdit(plano)} className="flex items-center gap-1 text-xs text-slate-500 hover:text-indigo-600">
                      <Pencil className="h-3.5 w-3.5" /> Editar
                    </button>
                    <button onClick={() => handleDelete(plano)} className="flex items-center gap-1 text-xs text-slate-500 hover:text-red-600">
                      <Trash2 className="h-3.5 w-3.5" /> Remover
                    </button>
                  </div>
                </div>
              ))}
              {planos.length === 0 && <p className="text-sm text-slate-400 col-span-full text-center py-8">Nenhum plano cadastrado ainda.</p>}
            </div>
          )}
        </div>

        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <h2 className="font-bold text-slate-800 mb-4">Atribuir plano às lojas</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {empresas.map((emp) => (
                  <tr key={emp.id}>
                    <td className="py-2.5 pr-3 font-medium text-slate-700">{emp.nome}</td>
                    <td className="py-2.5 pr-3 text-slate-500">{emp.comissaoPercent}%</td>
            ...
            {empresas.length === 0 && <p className="text-sm text-slate-400 text-center py-8">Nenhuma empresa cadastrada ainda.</p>}
          </div>
        </div>
      </div>
      </div>
