    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="mx-auto max-w-4xl px-4 py-6 sm:px-6 lg:px-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {planos.length > 0 && (
            <section className="rounded-2xl border border-slate-200 bg-white p-5">
              <h3 className="mb-3 flex items-center gap-1.5 text-sm font-bold text-slate-800">
                <Layers className="h-3.5 w-3.5 text-indigo-600" /> Plano de assinatura
              </h3>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                <label
                  className={`flex cursor-pointer flex-col rounded-xl border px-3.5 py-3 transition-colors ${
                    !form.planoId ? 'border-indigo-500 bg-indigo-50/60' : 'border-slate-200 hover:bg-slate-50'
                  }`}
                >
                  <div className="flex items-center gap-2">
                    <input type="radio" name="planoId" checked={!form.planoId} onChange={() => set('planoId', '')} />
                    <span className="text-sm font-semibold text-slate-800">Sem plano</span>
                  </div>
                  <span className="mt-1 text-xs text-slate-400">Comissão avulsa sobre as vendas</span>
                    ...
                    <div className="mt-2.5 flex items-center gap-1.5">
                      <div className="relative">
                        <input
                          ...
                          className="w-20 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-sm text-slate-900 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                        />
                        <Percent className="pointer-events-none absolute right-2 top-1/2 h-3 w-3 -translate-y-1/2 text-slate-400" />
                      </div>
                      <span className="text-[11px] text-slate-400">(1 a 20%)</span>
                    </div>
                  )}
                ...
                {planos.filter((p) => p.ativo).map((plano) => (
                  <label
                    ...
                    className={`relative flex cursor-pointer flex-col rounded-xl border px-3.5 py-3 transition-colors ${
                      form.planoId === plano.id ? 'border-indigo-500 bg-indigo-50/60' : 'border-slate-200 hover:bg-slate-50'
                    }`}
                  >
                    {plano.destaque && (
                      <span className="absolute -top-2 right-3 flex items-center gap-1 rounded-full bg-indigo-600 px-2 py-0.5 text-[9px] font-bold text-white">
          ...
          <section className="flex flex-wrap gap-6 rounded-2xl border border-slate-200 bg-white px-4 py-4">
